...
function downloadFile($db, $id, $user) {
    ...
    // Build absolute path using DOCUMENT_ROOT
    $dbPath = $file['path'];
    $documentRoot = $_SERVER['DOCUMENT_ROOT'];

    // Remove leading ../ from DB path
    if (strpos($dbPath, '../') === 0) {
    ...
    // Build and normalize absolute path
    $filePath = realpath($documentRoot . '/../' . $relativePath);
...
function viewFile($db, $id, $user) {
    ...
    // Build absolute path to avoid issues with current working directory
    // DB path is like: ../allfiles/pf-archives/file.pdf
    // We need absolute path to the file

    // Get the document root (usually /home/username/public_html/e-file.lerumaenterprises.co.tz)
    $documentRoot = $_SERVER['DOCUMENT_ROOT'];
    ...
    // Build absolute path: /home/username/public_html/allfiles/pf-archives/file.pdf
    ...
    if (!$filePath || !file_exists($filePath)) {
        ...
        ApiResponse::error('Physical file not found. Document Root: ' . $documentRoot . ', DB Path: ' . $dbPath . ', Attempted Path: ' . ($filePath ?: 'realpath failed'), 404);
    }
